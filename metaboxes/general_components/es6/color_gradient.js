Vue.component('wpcfto_color_gradient', {
	...
	created: function () {
		...
		this.gradient = {
			from: {
				input_value:
					typeof this.field_value.from !== 'undefined'
						? this.field_value.from
						: '',
				value:
					typeof this.field_value.from !== 'undefined'
						? this.field_value.from
			...
			to: {
				input_value:
					typeof this.field_value.to !== 'undefined' ? this.field_value.to : '',
				value:
					typeof this.field_value.to !== 'undefined' ? this.field_value.to : '',
			},
		}
		...
		gradient: {
			...
			handler: function (gradient) {
				...
				Object.keys(gradient).forEach(key => {
					if (
						gradient[key].input_value !== this.copy_gradient[key].input_value
					) {
						value[key] = gradient[key].input_value
						this.$set(gradient[key], 'value', value[key])
					} else {
						value[key] =
							typeof gradient[key].value === 'string'
								? gradient[key].value
								: this.vuecolor_to_string(gradient[key].value)
